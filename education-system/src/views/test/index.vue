    ...
    <div class="career-quiz-container container">
        ...
        <form @submit.prevent="submitQuiz">
            ...
            <div class="question-card">
                <h3>Q1. Which school subjects do you actually enjoy the most? <small>(Pick up to 3)</small></h3>
                <div class="options-grid">
                    <div v-for="(option, index) in questions[0].options" :key="index" class="option-item">
                        ...
                        <label :for="'q1-option' + index">
                            {{ option.text }}<!--  <span class="career-tag">{{ option.tags.join(', ') }}</span> -->
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content">
                                    ...
                                    <div class="card h-100 border-primary shadow-sm">
                                        ...
                                        <div class="card-body">
                                            <div class="progress mb-3" style="height:20px">
                                                <div class="progress-bar"  :class="getProgressBarClass(index)"
                                                    role="progressbar" :style="{ width: cluster.score + '%' }">
                                                    {{cluster.score + '%'}}
                                                </div>
                                                
                                            </div>
                                            <h6 class="text-primary">Recommended Majors:</h6>
                                            <div class="d-flex flex-wrap gap-2 mb-3">
                                                <span v-for="(job, jIndex) in cluster.jobs" :key="jIndex"
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" @click="showResults = false">Close</button>
                        <button type="button" class="btn btn-primary" @click="restartQuiz">Re-Test</button>
                    </div>
                </div>
            </div>
        </div>
...
const calculateResults = () => {
    ...
    careerClusters.value = careerClustersData.map(cluster => {
        const clusterScore = cluster.tags.reduce((sum, tag) => sum + (scoreMap[tag] || 0), 0)
